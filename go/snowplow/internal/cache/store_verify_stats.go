...
type StoreVerificationStats struct {
	...
	GVRsUndecorated           int64   `stat:"store_gvrs_undecorated" kind:"gauge" desc:"Registered GVRs whose informer did not get the verifying ListerWatcher, so the snapshot path is blind to them and only the forced pass covers them. Healthy baseline is the four typed-RBAC GVRs, which take the stock factory informer by design. DURING the coverage cliff (RESOLVER_COMPOSITION_STREAMING_LIST=false): jumps to nearly every GVR while every store_divergent_*_snapshot_total silently drops to 0 — which would otherwise read as health. A jump here is what names that."`
	...
	RepairUnsupportedTotal uint64  `stat:"store_repair_unsupported_total" desc:"Divergences whose GVR CANNOT be repaired by a relist, because its informer comes from the shared dynamic factory — which has no eviction API, so a teardown plus re-register hands back the STOPPED informer and kills the cache for that GVR instead of repairing it. Healthy: 0, and in production it should stay 0 because every non-typed-RBAC GVR takes the streaming path. Non-zero is the honest statement that this GVR is DETECTED and NOT repaired — a state that would otherwise look identical to a divergence nobody acted on."`
